 ...
 * $Id: bcmutils.c 471782 2014-04-22 00:37:42Z swatis $
 */
...
BCMATTACHFN(bcm_alloc_rxcplid_list)(osl_t *osh, uint32 cplid_max)
{
	...
	printf(rstr_allocating_rxcplid_buffers, cplid_max);
	...
	if (g_rxcplid_list == NULL) {
		printf(rstr_rxcplid_list_allocation_fail, size, cplid_max);
		return FALSE;
	}
...
BCMATTACHFN(pktpool_init)(osl_t *osh, pktpool_t *pktp, int *pplen, int plen, bool istx, uint8 type)
{
	...
	for (i = 0; i < pktplen; i++) {
		void *p;
#ifdef _HNDRTE_
		/* For hndrte builds, use PKTALLOC rather than PKTGET
...
/* XXX: Force pktpool_setmaxlen () into RAM as it uses a constant
...
static char *
#if defined(BCMROMBUILD) || defined(WLTEST) || defined(BCMINTERNAL) || \
	defined(BCMDBG_DUMP) || !defined(WLC_HIGH) || defined(ATE_BUILD)
getvar_internal(char *vars, const char *name)
#else
BCMATTACHFN(getvar_internal)(char *vars, const char *name)
#endif /* WLTEST || BCMINTERNAL || BCMDBG_DUMP || !WLC_HIGH */
{
#ifdef	_MINOSL_
	return NULL;
#else
