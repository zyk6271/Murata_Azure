    ...
    iot_sample_environment_variables* out_env_vars)
{
  ...
  extern char* endpoint_adress;
  extern char* device_id;
  out_env_vars->hub_hostname = AZ_SPAN_FROM_BUFFER(iot_sample_hub_hostname_buffer);
  read_configuration_entry(IOT_SAMPLE_ENV_HUB_HOSTNAME,endpoint_adress,true,out_env_vars->hub_hostname, &(out_env_vars->hub_hostname));
  out_env_vars->hub_device_id = AZ_SPAN_FROM_BUFFER(iot_sample_hub_device_id_buffer);
  read_configuration_entry(IOT_SAMPLE_ENV_HUB_DEVICE_ID,device_id,true,out_env_vars->hub_device_id,&(out_env_vars->hub_device_id));
}
