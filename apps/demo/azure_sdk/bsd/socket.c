...
int_wifi_socket_t *socket_add(void *ptr, int type) {
    ...
    if ( s ) {
        ...
        while( s->next ) { printf("try add %d", s->socket);  s = s->next;   if ( tmp->socket == s->socket ) tmp->socket++; }
        s->next = tmp;
    } else {
        tmp->socket = 0;
        socket_list = tmp;
...
struct hostent* gethostbyname(const char *name) {
  ...
  if ( r != WICED_SUCCESS ) { printf("gethostbyname failed %d", r); return NULL; }
  ...
  switch(socket_type) {
    ...
    case SOCK_DGRAM: {
        ...
        if ( (r = wiced_udp_create_socket(udp_socket, 5000 + rand()%45000, WICED_STA_INTERFACE)) != WICED_SUCCESS) {
            printf("Client Connection failed %d", r);
...
ssize_t send(int sockfd, const void *buf, size_t len, int flags) {
  ...
  if ( ( result = wiced_packet_create_tcp(ws->sock_space, TCP_PACKET_MAX_DATA_LENGTH, &packet, (uint8_t**)&tx_data, &available_data_length) ) != WICED_SUCCESS) {
      printf("TCP packet creation failed (%d, %d)", sockfd, result);
      return -1;
  }
//  printf("aval data len %d", available_data_length);
  ...
  if ( result != WICED_SUCCESS) {
      printf("TCP packet send failed (%d, %d)", sockfd, result);
      ...
      if ( s < len ) {
          ...
          if( result != WICED_SUCCESS ) {
              if ( rx_packet ) {
                  ...
                  printf("TCP packet reception failed");
              }
              return s==0?-1:s;
          }
...
ssize_t recvfrom(int sock, void *buf, size_t size, int flags,
  ...
  if ( ( result == WICED_ERROR ) || ( result == WICED_TIMEOUT ) ) {
      printf("Receive datagram failed %d", result);
      return -1;
  }
  if ( packet ) {
...
int connect(int socket,
  ...
  if ( !ws ) return -1;
  const wiced_ip_address_t INITIALISER_IPV4_ADDRESS( target_ip_addr, rem_addr->sin_addr.s_addr );
//  printf("try to connect %08x : %d", target_ip_addr.ip.v4, htons(rem_addr->sin_port));
  wiced_result_t result = wiced_tcp_connect( ws->sock_space, &target_ip_addr, htons(rem_addr->sin_port), ws->timeout );
  if ( result != WICED_SUCCESS ) {
      printf("TCP connaction fail %d", result);
      return -1;
  }
  return 0;
}
...
//                printf("set timeout %d", ws->timeout);
            }
          }
          return 0;
        }
